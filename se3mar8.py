...
from utils.data_utils import SE3Demo, seg_pointcloud, random_dropout, mask_part_point_cloud, get_heatmap
...
# Set up experiment directory and data path
wd = os.path.join("experiments", "mug", "pick")
os.makedirs(wd, exist_ok=True)
demo_path = os.path.join("data", "mug", "pick", "demo.npz")
# Alternatively: demo_path = os.path.join("data", "mug", "pick", "demo.npz")
...
# Define the model (segmentation version)
class Model(nn.Module):
    def __init__(self, voxelize=True, voxel_size=0.01, radius_threshold=0.12, feature_point_radius=0.02):
        ...
        self.pos_net = SE3Backbone(
            fiber_out=Fiber({"0": 1}),  # one heatmap
            num_layers=4,
            num_degrees=3,
            num_channels=8,
            num_heads=1,
    ...
    def forward(self, inputs, train_pos=False, reference_point=None, distance_threshold=0.3, random_drop=False, draw_pcd=False, pcd_name=None, mask_part=False, save_ori_feature=False):
        ...
        for i in range(bs):
            if draw_pcd:
                ...
                save_pcd_as_pcd(inputs["xyz"][i],
                                gt_heatmaps[-1].unsqueeze(-1).repeat(1, 3) / torch.max(gt_heatmaps[-1]),
                                save_file=f"pcd/mani/gt_heatmap_{pcd_name}_{i}.pcd", draw_heatmap=True)
            if reference_point is not None:
                data = seg_pointcloud(inputs["xyz"][i], inputs["rgb"][i], reference_point[i], distance=distance_threshold)
            else:
                data = {"xyz": inputs["xyz"][i], "rgb": inputs["rgb"][i]}
            if random_drop:
        ...
        # Process position output
        if train_pos:
            seg_output = self.pos_net(inputs)
            xyz = seg_output["xyz"]
            feature = seg_output["feature"]
            pos_weights = []
            output_pos = torch.zeros([len(xyz), 3]).to(self.pos_net.device)
            for i in range(len(xyz)):
                if draw_pcd:
                    save_pcd_as_pcd(xyz[i], feature[i][:, 0].clone().unsqueeze(-1).repeat(1, 3) / torch.max(feature[i][:, 0].clone()),
                                      save_file=f"pcd/mani/pos_heatmap_{pcd_name}_{i}.pcd", draw_heatmap=True)
                pos_weight = F.softmax(feature[i].reshape(-1, 1), dim=0).squeeze()
        ...
        if draw_pcd:
            for i in range(len(xyz)):
                ...
                closest_point_idx = torch.argmin(distances)
                save_pcd_as_pcd(xyz[i], seg_output["given_graph"]["raw_node_feats"][i][:, :3],
                                save_file=f"pcd/mani/ball_{pcd_name}_{i}.pcd")
        ori_output = self.ori_net(inputs)
        xyz = ori_output["xyz"]
        feature = ori_output["feature"]    # 3*3 = 9
        output_ori = torch.zeros([len(xyz), 9]).to(self.ori_net.device)
        if save_ori_feature:
            for i in range(len(xyz)):
...
# Training loop for segmentation network
for epoch in range(cfg.epoch):
    ...
    for i, data in enumerate(progress_bar):
        optm.zero_grad()
        output_pos = model({"xyz": data["xyz"], "rgb": data["rgb"]},
    ...
    with torch.no_grad():
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(model.state_dict(), os.path.join(wd, "segnet.pth"))
            print("Model saved!")
    scheduler.step()

# --- Train Mani (Manipulation) Network --- #

# Update experiment and demo path for mani training
all_cfg = OmegaConf.load("config/mug/pick/config.json")
cfg = all_cfg.mani
cfg_seg = all_cfg.seg
...
class Model_mani(nn.Module):
    ...
    def forward(self, inputs, train_pos=False, reference_point=None, distance_threshold=0.3, random_drop=False, draw_pcd=False, pcd_name=None, mask_part=False, save_ori_feature=False):
        ...
        if train_pos:
            ...
            for i in range(len(xyz)):
                if draw_pcd:
                    save_pcd_as_pcd(xyz[i],
                                      feature[i][:, 0].clone().unsqueeze(-1).repeat(1, 3) / torch.max(feature[i][:, 0].clone()),
                                      save_file=f"pcd/mani/pos_heatmap_{pcd_name}_{i}.pcd", draw_heatmap=True)
                pos_weight = F.softmax(feature[i].reshape(-1, 1), dim=0).squeeze()
                output_pos[i] = (xyz[i].T * pos_weight).T.sum(dim=0)
                pos_weights.append(pos_weight)
...
# Instantiate the mani model
model_mani = Model_mani()
model_mani.to("cpu")

# Define optimizer, scheduler, and loss for mani training
optm = torch.optim.Adam(model_mani.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
scheduler = StepLR(optm, step_size=int(cfg.epoch/5), gamma=0.5)
loss_fn = nn.MSELoss()
best_test_loss = 1e5

# Training loop for mani network
for epoch in range(cfg.epoch):
    progress_bar = tqdm(train_loader, desc=f"Epoch {epoch}/{cfg.epoch}")
    model_mani.train()
    for i, data in enumerate(progress_bar):
        optm.zero_grad()
        with torch.no_grad():
            ref_point = model_mani({"xyz": data["xyz"], "rgb": data["rgb"]}, random_drop=False)
    ...
    with torch.no_grad():
        ...
        if test_pos_loss + test_ori_loss < best_test_loss:
            ...
            torch.save(model_mani.state_dict(), os.path.join(wd, "maninet.pth"))
            print("Model saved!")
    scheduler.step()
